//
// The evaluator (higher-order lazy).
//
...
//**************************************
// For lazy evaluation: we need to create and force thunks.
...
value_t *thunk_force(value_t *v)
{
  if (v->type == v_thunk) {
    if (thunk_env(v) == NULL) {
      ...
      thunk_val(v) = e_expr(thunk_env(v), thunk_expr(v));
      thunk_env(v) = NULL;
...
static int thunk_list_i(void *data, void *info)
{
  eli_closure_t *c = (eli_closure_t *)info;
  expr_t *expr = (expr_t *)data;

  list_cons(thunk_create(c->env, expr), c->list);
